...
def hit_monarch_api(node = 'HGNC:17646', rows = 2000):
    ...
    Readjusted for new monarch version by Mireia
    """
    
    # api address
    node_new = node.split(':')
    node_call = node_new[0]+'%3A'+node_new[1]+'&limit=500'
    biolink = 'https://api-v3.monarchinitiative.org/v3/api/association'
    # https://api-v3.monarchinitiative.org/v3/api/association?subject=HGNC%3A17646&limit=500  ---what we want
    
    
    # parameters
    parameters = {'fl_excludes_evidence': False, 'rows': rows}
    # out edges: from/
    r_out = requests.get('{}?subject={}'.format(biolink,node_call),params=parameters)
    
    # in edges: to/
    r_in = requests.get('{}?object={}'.format(biolink,node_call),params=parameters)
...
def keep_node_type(edges, seed, nodeType='ortho'):
    ...
    propertyList = ['RO:HOM0000017', 'biolink:orthologous_to' 'RO:HOM0000020', ]
    if nodeType == 'pheno':
        propertyList = ['RO:0002200', 'biolink:has_phenotype', 'RO:0002607',  'biolink:biomarker_for', 'RO:0002326', 
                        'biolink:contributes_to', 'GENO:0000840', 'biolink:gene_associated_with_condition']
