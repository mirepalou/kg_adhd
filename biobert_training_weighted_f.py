...
def train(model, device, optimizer, train_dataloader, num_labels, class_weights):
    '''
    ...
    return model, tr_loss, nb_tr_steps, epoch_loss_all
...
def evaluate(model, device, dataloader, num_labels, all_label_ids = None, tr_loss=0, nb_tr_steps=0):
    ...
    return model, preds, result, eval_loss_all
...
def save_model(model, tokenizer):
    ...
    save_directory = os.path.join(script_dir, 'saved_model_lr5')
...
def main():
    ...
    # TRAIN
    train_examples = pd.read_csv('train_neg_tt.csv', sep='\t')
    ...
    # save mapping
    with open('map_rel_lr5.txt', 'w') as file:
        file.write(json.dumps(label_map))

    # VALIDATION
    eval_examples_0 = pd.read_csv('val_neg_tt.csv', sep='\t')
    ...
    class_weights_tensor = torch.tensor(class_weights, dtype=torch.float).to(device) # Convert the class weights to a tensor

    # 1 training
    print('Training')
    model, tr_loss, nb_tr_steps, epoch_loss_all = train(model, device, optimizer, train_dataloader, num_labels, class_weights_tensor)
    
    with open('epoch_loss_lr5.txt', 'w') as file:
        file.write(json.dumps(epoch_loss_all))

    # 2 validation
    print('Validation')
    model, preds_val, result, eval_loss_all = evaluate(model, device, eval_dataloader, num_labels, all_label_ids_e, tr_loss, nb_tr_steps)
    save_predictions(preds_val, eval_examples, 'val_preds_lr5.csv')
    
    with open('eval_loss_lr5.txt', 'w') as file:
        file.write(json.dumps(eval_loss_all))
